...
class employerInsurance:
    ...
    def __init__(self):
        ...
        self.host_url = self.api_url_datas['test_url']  # 环境地址
    ...
    def test_02_save_basic_info(self):
        ...
        if self.channelCategoryCode == 'A':
            request_data['channel']['secondPartnerId'] = '1057'
            request_data['channel']['secondPartnerFullName'] = '众安官网'
        else:
        ...
        if 'error' in save_response.keys() or ('success' in save_response.keys() and save_response['success'] != True):
            # 已保存过数据，重新请求获取入参
            findSalesChannel_url = self.api_url_datas['findSalesChannel'].format(self.host_url, self.apply_id)
